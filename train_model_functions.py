...
def regularization_loss_term_2(occupancy_values_on_ray):
    ...
    Returns:    One regularization loss term, the one for this ray."""
    return torch.mean(
        torch.min(occupancy_values_on_ray**2, (1 - occupancy_values_on_ray) ** 2)
    )
...
def regularization_loss_term_6(dataset, model, ray, img_idx, r, c, A):
    ...
    selected_centers = ray[non_zero_indices]
    selected_norms = F.normalize(input_grad[non_zero_indices], p=2, dim=1)
    nearest_points, nearest_distances = find_k_nearest_neighbors(
        dataset, model, selected_centers, img_idx, r, c, A
    )
    ...
    # output_grad_norms = torch.norm(output_grad, p=2, dim=-1, keepdim=True)
    # output_grad = torch.where(output_grad_norms > 0, output_grad / output_grad_norms, output_grad)
    output_grad = compute_norms(model, nearest_points.view(-1, 3), A).view(
        nearest_points.shape
    )
    output_grad_norms = torch.norm(output_grad, p=2, dim=-1, keepdim=True)
    output_grad = torch.where(
        output_grad_norms > 0, output_grad / output_grad_norms, output_grad
    )
    selected_norms = selected_norms.unsqueeze(1)
    loss = torch.mean(
        torch.mean(
            torch.div(
                torch.norm(output_grad - selected_norms, p=1, dim=2), nearest_distances
            ),
            dim=1,
        )
    )
...
def regularization_loss_term_eikonal_surface_aware(
...
def find_k_nearest_neighbors(dataset, model, selected_centers, img_idx, r, c, A):
    ...
    _, nearest_indices = distances.topk(
        min(6, selected_points.shape[0]), dim=1, largest=False, sorted=False
    )
    nearest_points = torch.gather(
        selected_points.expand(len(selected_centers), -1, -1),
        1,
