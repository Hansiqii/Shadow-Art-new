...
def train(current_dir, device, model, optimizer, loss_fn, epochs, batches, beta_1, beta_2, beta_3, beta_4, beta_5, beta_6):
    ...
    for epoch in range(epochs):
        ...
        save_angles_to_txt(angles, folder_path=os.path.join(current_dir, "outcomes"))
        save_angles_to_txt(screens, folder_path=os.path.join(current_dir, "outcomes"), filename="A_screens.txt")
        ...
        epoch_losses = train_loop(dataset, dataloader, ratio, model, optimizer, loss_fn, batches, beta_1 * 2 ** min(epoch, 3), beta_2 * 2 ** min(epoch, 3), beta_3 * (1 if 3 < epoch else 0)
, beta_4 * 15 ** (epoch), beta_5 * 2 ** (epoch), beta_6 * (1 if epoch > 3 else 0))
        ...
        draw_losses.append((avg_loss, avg_figure_loss, avg_regular_loss_1, avg_regular_loss_2, avg_regular_loss_3, avg_regular_loss_6))
        print(f"End of epoch {epoch + 1}, average loss: {avg_loss:>7f}")
        print(f"End of epoch {epoch + 1}, average figure loss: {avg_figure_loss:>7f}")
        print(f"End of epoch {epoch + 1}, average regular each near loss: {avg_regular_loss_1:>7f}")
        print(f"End of epoch {epoch + 1}, average regular near 0/1 loss: {avg_regular_loss_2:>7f}")
        print(f"End of epoch {epoch + 1}, average regular minimum volume loss: {avg_regular_loss_3:>7f}")
        ...
        print(f"End of epoch {epoch + 1}, average regular curvature loss: {avg_regular_loss_6:>7f}")
        output_dir = os.path.join(current_dir, 'outcomes')
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        torch.save({'model.state_dict': model.state_dict(), 'optimizer.state_dict': optimizer.state_dict()},
        ...
        for img_idx in range(len(dataset.images_and_angles)):
            compute_diff(current_dir, os.path.join(current_dir, f"data/input/{img_idx}.png"),
                         os.path.join(current_dir, f"outcomes/outcome_Figure{img_idx}_Epoch{epoch+1}.png"),
                         epoch, img_idx, 0)
            compute_diff(current_dir, os.path.join(current_dir, f"data/Figures/{img_idx}.png"),
                         os.path.join(current_dir, f"outcomes/outcome_Figure{img_idx}_Epoch{epoch+1}.png"),
                         epoch, img_idx, 2)
        if epoch % 5 == 4 and epoch > 5:
            print("registration!")
            for img_idx in range(len(dataset.images_and_angles)):
                registration(current_dir, os.path.join(current_dir, f"data/Figures/{img_idx}.png"), os.path.join(current_dir, f"data/temp/outcome_{img_idx}.png"), epoch, img_idx)
                compute_diff(current_dir,  os.path.join(current_dir, f"data/input/{img_idx}.png"),
                            os.path.join(current_dir,f"outcomes/Registration_Figure{img_idx}_Epoch{epoch+1}.png"),
                            epoch, img_idx, 1)
    return all_losses, draw_losses, real_figure_losses

def train_loop(dataset, dataloader, ratio, model, optimizer, loss_fn, batches=0, beta_1=0.1, beta_2=0.1, beta_3=5e-4, beta_4=0.1, beta_5 = 0.1, beta_6 = 0.1):
    ...
    for batch, (rays, occupancy_values, volumes, img_idxs, rs, cs) in enumerate(dataloader):
        if not batches == 0 and batch >= batches:
            break

        rays, occupancy_values, volumes, img_idxs, rs, cs = rays.to(device), occupancy_values.to(device), volumes.to(device), img_idxs.to(device), rs.to(device), cs.to(device)
        optimizer.zero_grad()
        ...
        for ray_id, ray in enumerate(rays):
            ...
            accumulated_occupancy = train_model_functions.accumulated_occupancy(occupancy_values_on_ray)
            occupancy_estimation[ray_id] = accumulated_occupancy
            regularisation_loss_1 += train_model_functions.regularization_loss_term_1(occupancy_values_on_whole_ray)
            regularisation_loss_2 += train_model_functions.regularization_loss_term_2(occupancy_values_on_whole_ray)
            volume = volumes[ray_id]
            volume = volume[truncate[1]]
            regularisation_loss_3 += train_model_functions.regularization_loss_term_3(occupancy_values_on_ray, volume, threshold=0.1, temperature = 1e-3)
            ...
            regularisation_loss_6 += train_model_functions.regularization_loss_term_6(dataset, model, ray, img_idx, r, c, A)
        ...
        loss = 2 * (beta_1 * regularisation_loss_1 + beta_2 * regularisation_loss_2 +
                beta_3 * regularisation_loss_3 + beta_4 * regularisation_loss_4 + beta_5 * regularisation_loss_5 + beta_6 * regularisation_loss_6 +img_loss)
        # loss = img_loss
        losses.append((float(loss), img_loss.item(),  float(regularisation_loss_1), float(regularisation_loss_2), float(regularisation_loss_3), float(regularisation_loss_4), float(regularisation_loss_5), float(regularisation_loss_6)))
