...
def registration(current_dir, image_1, image_2, epoch, img_idx):
    ...
    contours, _ = cv2.findContours(
        pre_image, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
    )
    # 提取所有黑色区域轮廓点
    black_points = np.vstack(contours).squeeze()  # 提取所有轮廓点并压缩维度
    # 对黑色区域的轮廓点进行变换
    transformed_black_points = cv2.transform(
        np.array([black_points], dtype="float32"), transformation_matrix
    )[0]

    # 检查黑色区域变换后是否出界
    if (
        (transformed_black_points[:, 0] >= 0).all()
        and (transformed_black_points[:, 0] < cols).all()
        and (transformed_black_points[:, 1] >= 0).all()
        and (transformed_black_points[:, 1] < rows).all()
    ):
        # 如果黑色区域未出界，执行变换
        warped_imageA = cv2.bitwise_not(
            cv2.warpAffine(imageA, transformation_matrix, (cols, rows))
        )
    ...
    cv2.imwrite(
        os.path.join(
            current_dir, f"outcomes/Registration_Figure{img_idx}_Epoch{epoch+1}.png"
        ),
        warped_imageA,
    )
